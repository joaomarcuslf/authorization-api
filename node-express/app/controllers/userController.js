...
class UserController {
  getUsers(request, response) {
    ...
    function fetchSuccess(data) {
      ...
      response.status(httpStatus.SUCCESS).send(filteredData);
    }
    ...
    userModel.findUsers().then(fetchSuccess).catch(fetchError);
  }
      ...
      .then(function(user) {
        ...
        let updatedUser = {
          ...
          password: request.body.password
            ? cryptHelper.generatePasswordHash(request.body.password)
            : user.username,
  ...
  deleteUserByUsername(request, response) {
    ...
    userModel
      ...
      .then(function() {
        let query = { username: username };
        userModel.delete(query).then(deleteSuccess).catch(deleteError);
      })
      .catch(deleteError);
  }
}
