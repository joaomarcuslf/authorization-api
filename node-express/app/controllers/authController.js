...
class AuthController {
  login(request, response) {
    ...
    let hasErrors = validationHelper.validateFields(request.body, [
    ...
    function authorizeSucess(data) {
      response.status(httpStatus.SUCCESS).send(genToken(data));
    }
    ...
    userModel.findUser(login).then(authorizeSucess).catch(authorizeError);
  }
  ...
  signup(request, response) {
    ...
    function insertSuccess(data) {
      response.status(httpStatus.SUCCESS).send(data);
    }
    ...
    userModel
      ...
      .then(insertError)
      .catch(function() {
        userModel.insert(user).then(insertSuccess).catch(insertError);
      });
  }
}
